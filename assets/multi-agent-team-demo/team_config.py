...
# 角色来源统一查「统一角色池」（references/统一角色池.md，335 个唯一角色 / 23 个领域）：
...
# 两类讨论分工：小组讨论 = 选哪个方案 + 理由；团队讨论 = 如何实施该方案
TASK_BRIEF = (
    "请围绕「{topic}」协作完成，严格按两级讨论收敛：\n"
    "1) 小组讨论（领域小组内部）：research / engineering / testing 各组充分比较思路，辩到收敛；\n"
    "2) 每组只输出 1 条：选定哪个方案 + 关键理由（禁止把分歧原样上抛）；\n"
    "3) 团队讨论（跨领域）：汇总各组建议，讨论如何落地——与既有代码衔接、职责划分、执行顺序，并解决跨域冲突；\n"
