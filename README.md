...
```mermaid
flowchart TD
  A["用户说「多Agent团队」"] --> B{"判定需求形态<br/>内部判断，不对外纠结"}

  B --> F1["形态 A<br/>现成专家人设"]
  B --> F2["形态 B<br/>代码搭 SDK"]
  B --> F3["形态 C<br/>开箱应用"]
  B --> F4["形态 D<br/>企业级框架"]
  ...
  subgraph P1["① 组队：先查池、后新建"]
    direction TB
    G["查统一角色池<br/>335 角色 / 23 领域"] --> G1{"池内有匹配角色？"}
    G1 -- 是 --> G2["复用其人设<br/>不重写 role / goal / backstory"]
    G1 -- 否 --> G3["才新建角色<br/>source 用 new: 前缀"]
    G2 --> G4["写入 TEAM<br/>带 domain + source"]
    G3 --> G4
    G4 --> G5["校验：每领域 ≥2 人"]
  end
  ...
  subgraph P2["② 两类讨论：小组选方案、团队定实施"]
    direction TB
    D["小组讨论 · 聚焦方案本身<br/>充分比较各 agent 的思路"] --> D1["每领域只出 1 条<br/>选哪个方案 + 关键理由"]
    D1 --> D2["团队讨论 · 聚焦如何融入任务<br/>衔接 · 职责划分 · 执行顺序"]
    D2 --> D3["产出：如何实施该方案<br/>解决跨域冲突，收敛唯一方案"]
  end

  D3 --> E

  subgraph P3["③ 落地运行：引擎可插拔"]
    direction TB
    E["选择运行方式"] --> E1["--dry-run<br/>零成本预览团队"]
    E --> E2["--engine mock<br/>零依赖演示全流程"]
    E --> E3["真实运行<br/>需 Key 或本地模型"]
  end
