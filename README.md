...
完整链路见 `references/团队搭建流程.svg`：

**用户说「多Agent团队」→ 判定需求形态（四类分支）→ 内部选型 → 组建领域小组（每领域 ≥2 人，从全量清单 608 招募）→ 小组讨论（比较思路 → 选哪个方案 + 理由）→ 团队讨论（衔接·职责·顺序 → 如何实施）→ 落地执行（能跑就执行，不能则给具体示例）→ 交付与安全提示。**
